YUI.add('moodle-mod_scheduler-studentattend', function (Y, NAME) {

var SELECTORS = {
        CHECKBOXES: 'input.studentattendselect'
    },
    MOD;
 
M.mod_scheduler = M.mod_scheduler || {};
MOD = M.mod_scheduler.studentattend = {};
...
MOD.save_status = function(cmid, appid, newseen, spinner) {
    ...
    Y.io(M.cfg.wwwroot + '/mod/scheduler/student_mark_ajax.php', {
...
MOD.init = function(cmid) {
	Y.all(SELECTORS.CHECKBOXES).each( function(box) {
		box.on('change', function(e) {
			...
			M.mod_scheduler.studentattend.save_status(cmid, box.get('value'), box.get('checked'), spinner);
		})
	});
};
