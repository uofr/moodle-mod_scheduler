 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_scheduler_notifyqueue {
    ...
    public static function show() {
        ...
        if (isset($SESSION->mod_scheduler_notifyqueue)) {
            foreach ($SESSION->mod_scheduler_notifyqueue as $message) {
                echo $OUTPUT->notification($message->message, 'notify'.$message->type);
            }
            unset($SESSION->mod_scheduler_notifyqueue);
        }
    }
    ...
    private static function queue_message($message, $messagetype=\core\output\notification::NOTIFY_INFO) {
        ...
        if (!isset($SESSION->mod_scheduler_notifyqueue)) {
            $SESSION->mod_scheduler_notifyqueue = array();
        }
        $m = new stdclass();
        $m->type = $messagetype;
        $m->message = $message;
        $SESSION->mod_scheduler_notifyqueue[] = $m;
    }
}
